...
public class PlantHardSpell extends AAction {
    private int baseDamage;
    private int statusDuration;

    public PlantHardSpell(String name, Element element, int baseDamage, int statusDuration, int coolDownDuration) {
    ...
    public void execute(AEntity user, AEntity target) {
        if (isReady()) {
            ...
            WeakenStatus weaken = new WeakenStatus(statusDuration, 0.4);
